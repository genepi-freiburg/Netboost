...
## CHG JO: make params
$MAKE = 'SHELL=/bin/bash; make --debug';
##$MAKE = 'SHELL=/bin/bash; make ';
...
unless (defined($max_singleton_id)){
  ## CHG JO: Var must be initialized.
  $max_singleton_id = 0;
  ...
  while(<A>){
    ## CHG JO: Else the last member of split has "\n" attached
    chomp;

    ## CHG JO: Split alone not working for different separators
    @_ = split(/[\s+\t]/, $_);
  ...
  print "XXX MAX SINGLETON: $max_singleton_id\n";
}
