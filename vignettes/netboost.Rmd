...
components. The primary analysis is than carried out on these summary measures
...
chromosome 18 for 100 patients.   
...
The `netboost()` function integrates all mayor analysis steps and generates 
multiple plots. In this step we also set analysis parameters:

 `stepno` defines the number of boosting steps taken

 `softPower` (if null, automatically chosen) the exponent in the transformation
of the correlation

 `minClusterSize` the minimal size of clusters, `nPC` the number of maximally 
...
softPower = 3L, minClusterSize = 10L, nPC = 2, scale = TRUE, MEDissThres = 0.25) 
dev.off()
```
...
explained by the aggregate measures (varExplained).
Dependent on the minimum proportion of variance explained set in the 
`netboost()` call (default 0.5) up to `nPC` principal components are exported.
...
Explained variance is reported by a matrix for the first `nPC` principal 
